...
In class "cz.muni.fi.akka.twophasecommit.main.Main" change the variable "TRANSACTION_DECISION" for TransactionDecision.commit
if you want the participant to commit or TransactionDecision.abort for aborting.

In class cz.muni.fi.akka.twophasecommit.main.LockFileDemo change the FILE_PATH to the path of the file being locked.
...
In class "cz.muni.fi.akka.twophasecommit.main.Main" set the variable "PARTICIPANT_NAMES" to an array of 'p's with indexes.
(Arrays.asList("p1", "p2"))
...
/activator "run-main cz.muni.fi.akka.twophasecommit.main.Main participant"		

and then the coordinator:

/activator "run-main cz.muni.fi.akka.twophasecommit.main.Main coordinator"		